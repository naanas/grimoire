...
export default function AdminChatPage() {
    ...
    return (
        <div className="flex flex-col md:flex-row h-[calc(100vh-140px)] md:h-[calc(100vh-120px)] bg-neutral-900 rounded-xl overflow-hidden border border-neutral-800">
            ...
            <div className={`flex-1 flex flex-col bg-neutral-950 ${!selectedSessionId ? 'hidden md:flex' : 'flex'}`}>
                {selectedSessionId ? (
                    <>
                        {/* Header */}
                        <div className="p-4 border-b border-neutral-800 bg-neutral-900 flex items-center gap-3">
                            ...
                            <h3 className="font-bold text-white truncate">
                                {sessions.find(s => s.id === selectedSessionId)?.user?.name || sessions.find(s => s.id === selectedSessionId)?.guestName || 'Chat'}
                            </h3>
                        </div>
                            ...
                            {messages.map((msg) => {
                                ...
                                return (
                                    <div key={msg.id} className={`flex ${isAdmin ? 'justify-end' : 'justify-start'}`}>
                                        <div className={`
                                            max-w-[85%] md:max-w-[70%] px-4 py-3 rounded-2xl text-sm
                        ...
                        {/* Input */}
                        <div className="p-3 md:p-4 bg-neutral-900 border-t border-neutral-800 flex gap-2">
                            <input
                                type="text"
                                value={inputMessage}
                                onChange={(e) => {
                                ...
                                className="flex-1 bg-neutral-950 text-white rounded-lg px-4 py-2 outline-none focus:ring-1 focus:ring-red-500 border border-neutral-800 text-sm"
                            />
                            <button
                                onClick={handleSend}
                                disabled={!inputMessage.trim()}
                                className="p-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition disabled:opacity-50 shrink-0"
                            >
                                <Send size={20} />
                            </button>
                        </div>
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-neutral-500 gap-4 p-4 text-center">
